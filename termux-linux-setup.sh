...
#  - ADDED: Discord & Spotify (ncspot) installation
#######################################################

# ============== CONFIGURATION ==============
TOTAL_STEPS=13          # Increased step count
CURRENT_STEP=0
DE_CHOICE="1"
DE_NAME="XFCE4"
...
step_apps() {
    ...
    # ===== Discord Installation (ARM64) =====
    echo -e "${YELLOW}[!] Attempting to install Discord (experimental)...${NC}"
    mkdir -p ~/discord
    wget -q --show-progress -O /tmp/discord.tar.gz "https://discord.com/api/download?platform=linux&format=tar.gz" &
    spinner $! "Downloading Discord"
    tar -xzf /tmp/discord.tar.gz -C ~/discord --strip-components=1 > /dev/null 2>&1 &
    spinner $! "Extracting Discord"
...
EOF
    ...
    echo -e "${GREEN}[+] Discord placed in ~/discord. If it fails to run, you may need additional libraries.${NC}"

    # ===== Spotify (ncspot) =====
    echo -e "${YELLOW}[!] Installing ncspot (terminal Spotify client) as GUI client not available for ARM64...${NC}"
    install_pkg "ncspot" "ncspot (Spotify TUI)"
    mkdir -p ~/.local/share/applications
    cat > ~/.local/share/applications/ncspot.desktop << EOF
[Desktop Entry]
...
Terminal=true
EOF
    echo -e "${GREEN}[+] ncspot installed. Run 'ncspot' from terminal to configure and log in.${NC}"

    # ===== Notes on other browsers =====
    echo -e "${YELLOW}[!] Note: Opera/Opera GX do not provide ARM64 builds. Kiwi Browser is an Android app and cannot be installed inside Linux.${NC}"
}
...
EOF
    ...
    # Discord shortcut (if exists)
    if [ -f ~/discord/Discord ]; then
        cp ~/.local/share/applications/discord.desktop ~/Desktop/
    fi

    # ncspot shortcut
    cp ~/.local/share/applications/ncspot.desktop ~/Desktop/ 2>/dev/null
...
EOF
    ...
    echo -e "  [+] Added Firefox, VLC, Wine, Discord, ncspot, and Terminal shortcuts."
}

# ============== STEP 12: FINAL CLEANUP ==============
step_cleanup() {
    update_progress
    echo -e "${PURPLE}[Step ${CURRENT_STEP}/${TOTAL_STEPS}] Cleaning up...${NC}"
    echo ""
...
COMPLETE
    ...
    echo "    - Discord (experimental, in ~/discord)"
    echo "    - ncspot (terminal Spotify client)"
    echo ""
    echo -e "${YELLOW}------------------------------------------------------------${NC}"
    ...
    echo -e "${YELLOW}------------------------------------------------------------${NC}"
    echo ""
    echo -e "${YELLOW}[!] Note: If Discord fails to launch, you may need to install additional libraries:${NC}"
    echo "    Run 'ldd ~/discord/Discord' to see missing libraries, then install them via 'pkg install <library>'."
    echo -e "${YELLOW}[!] For Spotify, run 'ncspot' in a terminal to log in and enjoy music.${NC}"
    echo ""
}
